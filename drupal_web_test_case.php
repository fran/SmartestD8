...
class DrupalWebTestCase extends DrupalTestCase {
  ...
  protected function drupalCreateNode($settings = array()) {
    ...
    $settings += array(
      'body'      => array(\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED => array(array())),
  ...
  protected function drupalGetToken($value = '') {
    ...
    return \Drupal\Component\Utility\Crypt::hmacBase64($value, $this->session_id . $private_key);
  }
  ...
  protected function setUp() {
    ...
    include_once \Drupal::root() . '/includes/install.inc';
  ...
  protected function tearDown() {
    ...
    module_list(TRUE);
    \Drupal::moduleHandler()->getImplementations('', FALSE, TRUE);
  ...
  protected function drupalGetAJAX($path, array $options = array(), array $headers = array()) {
    return \Drupal\Component\Serialization\Json::decode($this->drupalGet($path, $options, $headers));
  }
  ...
  protected function drupalPostAJAX($path, $edit, $triggering_element, $ajax_path = NULL, array $options = array(), array $headers = array(), $form_html_id = NULL, $ajax_settings = NULL) {
    ...
    $return = \Drupal\Component\Serialization\Json::decode($this->drupalPost(NULL, $edit, array('path' => $ajax_path, 'triggering_element' => $triggering_element), $options, $headers, $form_html_id, $extra_post));
    ...
    if (!empty($ajax_settings) && !empty($return)) {
      ...
      foreach ($return as $command) {
        switch ($command['command']) {
          case 'settings':
            $drupal_settings = \Drupal\Component\Utility\NestedArray::mergeDeep($drupal_settings, $command['settings']);
            break;
    ...
    if (strpos($this->drupalGetContent(), '<meta ') && $this->parse()) {
      ...
      if (!empty($refresh)) {
        ...
        if (preg_match('/\d+;\s*URL=(?P<url>.*)/i', $refresh[0]['content'], $match)) {
          return $this->drupalGet($this->getAbsoluteUrl(\Drupal\Component\Utility\Html::decodeEntities($match['url'])));
        }
      }
    }
    return FALSE;
  ...
  protected function drupalSetContent($content, $url = 'internal:') {
    ...
    if (preg_match('/jQuery\.extend\(Drupal\.settings, (.*?)\);/', $content, $matches)) {
      $this->drupalSettings = \Drupal\Component\Serialization\Json::decode($matches[1]);
    }
  }
  ...
  protected function assertTextHelper($text, $message = '', $group, $not_exists) {
    if ($this->plainTextContent === FALSE) {
      $this->plainTextContent = \Drupal\Component\Utility\Xss::filter($this->drupalGetContent(), array());
    }
    if (!$message) {
  ...
  protected function assertThemeOutput($callback, array $variables = array(), $expected, $message = '', $group = 'Other') {
    ...
    $this->verbose('Variables:' . '<pre>' .  \Drupal\Component\Utility\SafeMarkup::checkPlain(var_export($variables, TRUE)) . '</pre>'
      ...
      . '<hr />' . 'Expected:' . '<pre>' .  \Drupal\Component\Utility\SafeMarkup::checkPlain(var_export($expected, TRUE)) . '</pre>'
      . '<hr />' . $output
    );
    if (!$message) {
